...
namespace GameStack.JWT.Builder
{
    ...
    public sealed class JwtBuilder
    {
        ...
        /// </summary>
        public static JwtBuilder Create() => new JwtBuilder();
        ...
        /// </summary>
        /// <remarks>
        /// If not set then default <see cref="JsonNetSerializer" /> will be
        /// used.
        /// </remarks>
        /// <returns>Current builder instance</returns>
        ...
        /// </summary>
        /// <remarks>
        /// If not set then default <see cref="UtcDateTimeProvider" /> will be
        /// used.
        /// </remarks>
        /// <returns>Current builder instance</returns>
        ...
        /// </summary>
        /// <remarks>
        /// If not set then default <see cref="JwtBase64UrlEncoder" /> will be
        /// used.
        /// </remarks>
        /// <returns>Current builder instance</returns>
        ...
        ///
        /// This was modified from its original form since Select is not
        /// available with the current .NET runtime Unity is using.
        /// </summary>
        /// <remarks>
        /// Required to create new token that uses an symmetric algorithm such
        /// as <seealso cref="RS256Algorithm" />
        /// </remarks>
        /// <returns>Current builder instance</returns>
        public JwtBuilder WithSecret(params string[] secrets)
        {
            ...
            {
                var secretBytes = GetBytes(secrets[i]);
                secretsBytes[i] = secretBytes;
            }
            _secrets = secretsBytes;
            return this;
        }
        ...
        public string Decode(string token)
        {
            ...
            return _verify ?
                _decoder.Decode(token, _secrets, _verify) :
        ...
        public T Decode<T>(string token)
        {
            ...
            return _verify ?
                _decoder.DecodeToObject<T>(token, _secrets, _verify) :
        ...
        /// <param name="token">The JWT</param>
        /// <returns>The payload converted to <see cref="T" /></returns>
        public T Decode<T>(string token, Func<string, T> jsonUnmarshaler)
        {

            EnsureCanDecode();

            return _verify ? _decoder.DecodeToObject<T>(token, _secrets,
                _verify, jsonUnmarshaler) : _decoder.DecodeToObject<T>(token,
                    jsonUnmarshaler);
        }
        ...
        private void TryCreateEncoder()
        {
            ...
            _encoder = new JwtEncoder(_algorithm, _urlEncoder);
        }
        ...
        private void TryCreateDecoder()
        {
            ...
            if (_algorithm is object)
                _decoder = new JwtDecoder(_validator, _urlEncoder, _algorithm);
            else if (_algFactory is object)
                _decoder = new JwtDecoder(_validator, _urlEncoder, _algFactory);
            else if (!_verify)
                _decoder = new JwtDecoder(_urlEncoder);
        }
        ...
        private void TryCreateValidator()
        {
            ...
            _validator = new JwtValidator(_dateTimeProvider);
        }
