...
namespace GameStack.JWT
{
    ...
    public sealed class JwtDecoder : IJwtDecoder
    {
        ...
        /// </summary>
        /// <remarks>
        /// This overload supplies no <see cref="IJwtValidator" /> and no
        /// <see cref="IAlgorithmFactory" /> so the resulting decoder cannot be
        /// used for signature validation.
        ///
        /// This has been modified from its original version to use the Unity
        /// JSON utility instead of the IJsonSerializer.
        /// </remarks>
        /// <param name="jsonSerializer">The Json Serializer</param>
        /// <param name="urlEncoder">The Base64 URL Encoder</param>
        /// <exception cref="ArgumentNullException" />
        public JwtDecoder(IBase64UrlEncoder urlEncoder)
        {
            _urlEncoder = urlEncoder ?? throw new ArgumentNullException(nameof(
                urlEncoder));
        }
        ...
        /// <exception cref="ArgumentNullException" />
        public JwtDecoder(IJwtValidator jwtValidator,
            IBase64UrlEncoder urlEncoder, IAlgorithmFactory algFactory)
            : this(urlEncoder)
        {
            _jwtValidator = jwtValidator ?? throw new ArgumentNullException(
                nameof(jwtValidator));
            _algFactory = algFactory ?? throw new ArgumentNullException(nameof(
                algFactory));
        }
        ...
        /// <exception cref="ArgumentNullException" />
        public JwtDecoder(IJwtValidator jwtValidator,
            IBase64UrlEncoder urlEncoder, IJwtAlgorithm algorithm)
            : this(jwtValidator, urlEncoder, new DelegateAlgorithmFactory(
                algorithm))
        {
        }
        ...
        public T DecodeHeader<T>(JwtParts jwt)
        {
            ...
            return JsonUtility.FromJson<T>(stringHeader);
        }
            ...
            if (verify)
            {
                if (_jwtValidator is null)
                    throw new InvalidOperationException(
                        "This instance was constructed without validator so cannot be used for signature validation");
                if (_algFactory is null)
                    throw new InvalidOperationException(
                        "This instance was constructed without algorithm factory so cannot be used for signature validation");
        ...
        public string Decode(JwtParts jwt, byte[][] keys, bool verify)
        {
            ...
            if (verify)
            {
                if (_jwtValidator is null || _algFactory is null)
                    throw new InvalidOperationException(
                        "This instance was constructed without validator and algorithm so cannot be used for signature validation");
        ...
        /// <exception cref="FormatException" />
        public T DecodeToObject<T>(JwtParts jwt,
            Func<string, T> jsonUnmarshaler)
        {
            var payload = Decode(jwt);
            return jsonUnmarshaler(payload);
        }
        ...
        /// </summary>
        /// <param name="parts">The array representation of a JWT</param>
        /// <param name="keys">The keys provided which one of them was used to
        /// sign the JWT</param>
        /// <exception cref="ArgumentNullException" />
        /// <exception cref="ArgumentOutOfRangeException" />
        /// <exception cref="FormatException" />
        ...
        public void Validate(JwtParts jwt, params byte[][] keys)
        {
            ...
            var alg = _algFactory.Create(JwtDecoderContext.Create(header,
                decodedPayload, jwt));
            ...
            {
                _jwtValidator.Validate(decodedPayload, asymmAlg, bytesToSign,
                    decodedSignature);
            }
            else
            {
                ...
                // signatures re-created by the algorithm, with the leading =
                var recreatedSignatures = keys.Select(
                    key => Convert.ToBase64String(alg.Sign(key,
                        bytesToSign))).ToArray();

                _jwtValidator.Validate(decodedPayload, rawSignature,
                    recreatedSignatures);
            }
        }
